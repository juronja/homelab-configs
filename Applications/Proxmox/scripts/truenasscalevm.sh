...
# Create a VM
qm create $VMID --cores $CORE_COUNT --cpu host --memory $RAM --balloon 0 --name truenas-scale --machine q35 --bios ovmf --efidisk0 local-lvm:1,efitype=4m,pre-enrolled-keys=1 --scsihw virtio-scsi-single --scsi0 local-lvm:$DISK_SIZE,ssd=on,iothread=on --cdrom local:iso/TrueNAS-SCALE-$SCALE_VRS.iso --net0 virtio,bridge=vmbr0,firewall=1 --ipconfig0 ip=dhcp,ip6=dhcp --agent enabled=1 --onboot 1
